...
@interface YZHUIHoverView () <UIGestureRecognizerDelegate,UICollectionViewDelegate, UICollectionViewDataSource>
...
-(void)_setupDefaultValue
{
    ...
    _isExpand = NO;
    self.flexDirection = YZHUIHoverViewFlexDirectionAny;
...
-(UICollectionView*)collectionView
{
    if (_collectionView== nil) {
        ...
        [self.collectionView registerClass:[YZHUIHoverActionCell class] forCellWithReuseIdentifier:NSSTRING_FROM_CLASS(YZHUIHoverActionCell)];
    }
...
-(void)_setupChildView
{
    ...
    _hoverButton = [YZHUIButton buttonWithType:UIButtonTypeCustom];
...
-(void)_hoverBtnAction:(YZHUIButton*)button
{
...
-(NSDictionary*)_getSelectFrameInfo
{
    ...
    YZHUIHoverViewFlexDirection flexDir = YZHUIHoverViewFlexDirectionRight;
    CGFloat rotateAngle = 0;
    
    if (frame.origin.x + max < showInViewSize.width && TYPE_AND(self.flexDirection, YZHUIHoverViewFlexDirectionRight)) {
        frame.size = CGSizeMake(max, frame.size.height);
        flexDir = YZHUIHoverViewFlexDirectionRight;
        rotateAngle = M_PI_2;
    }
    else if (frame.origin.x - max + min > 0 && TYPE_AND(self.flexDirection, YZHUIHoverViewFlexDirectionLeft))
    {
        frame = CGRectMake(frame.origin.x - max + min, frame.origin.y, max, frame.size.height);
        flexDir = YZHUIHoverViewFlexDirectionLeft;
        rotateAngle = -M_PI_2;
    }
    else if (frame.origin.y + max < showInViewSize.height && TYPE_AND(self.flexDirection, YZHUIHoverViewFlexDirectionDown))
    {
        frame.size = CGSizeMake(frame.size.width, max);
        flexDir = YZHUIHoverViewFlexDirectionDown;
        rotateAngle = -M_PI;
    }
    else if (frame.origin.y - max + min > 0 && TYPE_AND(self.flexDirection, YZHUIHoverViewFlexDirectionUp))
    {
        frame = CGRectMake(frame.origin.x, frame.origin.y-max + min, frame.size.width, max);
        flexDir = YZHUIHoverViewFlexDirectionUp;
        rotateAngle = M_PI;
    }
    else
    {
        if (TYPE_AND(self.flexDirection, YZHUIHoverViewFlexDirectionUp)) {
            max = MIN(showInViewSize.height, max);
            flexDir = YZHUIHoverViewFlexDirectionUp;
            frame = CGRectMake(frame.origin.x, 0, frame.size.width, max);
            rotateAngle = M_PI;
        }
        else if (TYPE_AND(self.flexDirection, YZHUIHoverViewFlexDirectionDown)) {
            max = MIN(showInViewSize.height, max);
            flexDir = YZHUIHoverViewFlexDirectionDown;
            frame = CGRectMake(frame.origin.x, showInViewSize.height - max, frame.size.width, max);
            rotateAngle = -M_PI;
        }
        else if (TYPE_AND(self.flexDirection, YZHUIHoverViewFlexDirectionRight)) {
            max = MIN(showInViewSize.width, max);
            flexDir = YZHUIHoverViewFlexDirectionRight;
            frame = CGRectMake(showInViewSize.width - max, frame.origin.y, max, frame.size.height);
            rotateAngle = M_PI_2;
        }
        else if (TYPE_AND(self.flexDirection, YZHUIHoverViewFlexDirectionLeft)) {
            max = MIN(showInViewSize.width, max);
            flexDir = YZHUIHoverViewFlexDirectionLeft;
...
-(void)_updateHoverView
{
    ...
    YZHUIHoverViewFlexDirection flexDir = [info[TYPE_STR(HoverViewFlexDirection)] integerValue];
    if (flexDir == YZHUIHoverViewFlexDirectionLeft || flexDir == YZHUIHoverViewFlexDirectionRight) {
        ...
        if (flexDir == YZHUIHoverViewFlexDirectionLeft) {
    ...
    else if (flexDir == YZHUIHoverViewFlexDirectionUp || flexDir == YZHUIHoverViewFlexDirectionDown)
    {
        ...
        if (flexDir == YZHUIHoverViewFlexDirectionUp) {
...
{
    YZHUIHoverActionCell *cell = [collectionView dequeueReusableCellWithReuseIdentifier:NSSTRING_FROM_CLASS(YZHUIHoverActionCell) forIndexPath:indexPath];
